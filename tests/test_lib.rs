use checksec::{elf, shared, checksec_core, shared::{BinResults, compress_results, decompress_results}};
mod utils;
...
fn test_roundtrip() {
    ...
    let compressed = compress_results(&result).expect("compress_results failed");
    let decompress_result = decompress_results(compressed.as_bytes()).expect("decompress_results failed");
